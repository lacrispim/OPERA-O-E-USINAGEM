...
export function TornoHoursBarChart({ records, className }: TornoHoursBarChartProps) {
  const chartData = useMemo(() => {
    ...
    records.forEach(record => {
      if (record.requestingFactory && dataByFactory.hasOwnProperty(record.requestingFactory)) {
        dataByFactory[record.requestingFactory] += record.tornoTime || 0;
      }
    });
